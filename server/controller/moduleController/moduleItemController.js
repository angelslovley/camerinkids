...
const deleteModuleItem = async (request, response) => {
  try {
    ...
    // Ensure result is a Mongoose document before calling remove
    if (result instanceof mongoose.Document) {
      deletedModule.moduleItems.pull(result);
    } else {
      // If result is not a Mongoose document, log a warning
      console.warn('Result is not a Mongoose document')
    }
